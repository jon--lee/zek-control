...
class XboxController:
    ...
    def __init__(self,scales = [.3,.01,.01,.01,.01]):
    ...
    def calibrate(self):
        ...
        #print (np.ones(length)-self.offsets)
        self.uScale = abs(length/(np.ones(length)-self.offsets))
        self.lScale = abs(length/(-np.ones(length)-self.offsets))
    ...
    def convert(self,state):
        ...
        #updates = np.array([rot,elev,ext,wrist,turntable])-self.offsets
